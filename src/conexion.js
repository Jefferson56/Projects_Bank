...
const {database}= require('./keys'); //Se importan los datos para conectar a la base de datos
...
pool.getConnection((err, connection) => { //Se obtiene la conexión
    if(err) { //Se evalúa si existe algún error con la conexión
        if(err.code === 'PROTOCOL_CONNECTION_LOST') { //Error, conexión cerrada
            console.error('DATABASE CONNETION WAS CLOSED');
        }
        if(err.code === 'ER_CON_COUNT_ERROR') { //Error, cantidad de conexiones con la base de datos
            console.error('DATABASE HAS TO MANY CONNECTIONS');
        }
        if(err.code === 'ECONNREFUSED') { //Error, conexión rechazada por la base de datos
