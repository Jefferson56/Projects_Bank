...
const {validarContrasena, validarTipoUsuario}= require('./validation.js');
...
passport.use('local.signup', new LocalStrategy({
    ...
    const {repit_password}= req.body;
    const {typeUser}= req.body;
     if(validarTipoUsuario(typeUser)){
        const newUser= {
            username,
            password,
            fullname,
        ...
        req.flash('message','Tipo de usuario no permitido');
        return done('', '');
     }
...
passport.deserializeUser(async (id, done) => { //
   const rows= await conexion.query('SELECT * FROM users WHERE id = ?', [id]); //Devuleve un arreglo con los datos de la consulta
   done(null, rows[0]); //rows[0] duvuelve el objeto en la posición cero del arreglo
});